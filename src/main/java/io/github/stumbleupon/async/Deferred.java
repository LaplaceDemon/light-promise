 ...
 */
package io.github.stumbleupon.async;
 ...
 * "pipelines" (processing chains) without having to explicitly define a finite
 * state machine.
 *
 * <h1>The tl;dr version</h1>
 *
 * We're all busy and don't always have time to RTFM in details. Please pay
 * special attention to the <a href="#warranty">invariants</a> you must respect.
 * Other than that, here's an executive summary of what {@code Deferred} offers:
 * <ul>
 * <li>A {@code Deferred} is like a {@link java.util.concurrent.Future} with a
 * dynamic {@link Callback} chain associated to it.</li>
 ...
 * <li>There are actually two callback chains. One is for normal processing, the
 * other is for error handling / recovery. A {@link Callback} that handles
 ...
 * and will hand its result (be it successful or not) to the {@code Deferred} in
 * the future. The key difference between a {@code Deferred} and a
 * {@link java.util.concurrent.Future Future} is that a {@code Deferred} has a
 * <strong>callback chain</strong> associated to it, whereas with just a
 * {@code Future} you need get the result manually at some point, which poses
 * problems such as: How do you know when the result is available? What if the
 * result itself depends on another future?
 * <p>
 * When you start an asynchronous operation, you typically want to be called
 * back when the operation completes. If the operation was successful, you want
 * your callback to use its result to carry on what you were doing at the time
 * you started the asynchronous operation. If there was an error, you want to
 * trigger some error handling code.
 * <p>
 ...
 *
 * <h2>Understanding the callback chain</h2> Let's take a typical example.
 * You're writing a client library for others to use your simple remote storage
 * service. When your users call the {@code
 * get} method in your library, you want to retrieve some piece of data from
 * your remote service and hand it back to the user, but you want to do so in an
 * asynchronous fashion.
 * <p>
 * When the user of your client library invokes {@code get}, you assemble a
 * request and send it out to the remote server through a socket. Before sending
 * it to the socket, you create a {@code Deferred} and you store it somewhere,
 * for example in a map, to keep an association between the request and this
 * {@code Deferred}. You then return this {@code Deferred} to the user, this is
 * how they will access the deferred result as soon as the RPC completes.
 * <p>
 * Sooner or later, the RPC will complete (successfully or not), and your socket
 * will become readable (or maybe closed, in the event of a failure). Let's
 * assume for now that everything works as expected, and thus the socket is
 * readable, so you read the response from the socket. At this point you extract
 * the result of the remote {@code get} call, and you hand it out to the
 * {@code Deferred} you created for this request (remember, you had to store it
 * somewhere, so you could give it the deferred result once you have it). The
 * {@code Deferred} then stores this result and triggers any callback that may
 * have been added to it. The expectation is that the user of your client
 * library, after calling your {@code get} method, will add a {@link Callback}
 * to the {@code Deferred} you gave them. This way, when the deferred result
 * becomes available, you'll call it with the result in argument.
 * <p>
 ...
 * {@code get} call. It will create a {@code Deferred}, and immediately hand it
 * the cached result, and it will return this {@code Deferred} to the user. The
 * user will add a {@link Callback} to it, which will be immediately invoked
 * since the deferred result is already available. So the entire {@code get}
 * call completed virtually instantaneously and entirely from the same thread.
 * There was no context switch (no other thread involved, no I/O and whatnot),
 * nothing ever blocked, everything just happened really quickly.
 * <p>
 * Now let's assume that the caching library has a cache miss and needs to do a
 * remote {@code get} call using the original client library described earlier.
 * The RPC is sent out to the remote server and the client library returns a
 * {@code Deferred} to the caching library. This is where things become
 * exciting. The caching library can then add its own callback to the
 * {@code Deferred} before returning it to the user. This callback will take the
 * result that came back from the remote server, add it to the cache and return
 * it. As usual, the user then adds their own callback to process the result. So
 * now the {@code Deferred} has 2 callbacks associated to it:
 * 
 * <pre>
 ...
 * caching library. Then whatever the first callback returns will be passed on
 * to the second callback. It turns out that the caching callback returns the
 * {@code get} response unchanged, so that will be passed on to the user
 * callback.
 * <p>
 * Now it's very important to understand that the first callback could have
 * returned another arbitrary value, and that's what would have been passed to
 * the second callback. This may sound weird at first but it's actually the key
 * behind {@code Deferred}.
 * <p>
 * To illustrate why, let's complicate things a bit more. Let's assume the
 * remote service that serves those {@code get} requests is a fairly simple and
 * low-level storage service (think {@code memcached}), so it only works with
 * byte arrays, it doesn't care what the contents is. So the original client
 * library is only de-serializing the byte array from the network and handing
 * that byte array to the {@code Deferred}.
 * <p>
 * Now you're writing a higher-level library that uses this storage system to
 * store some of your custom objects. So when you get the byte array from the
 * server, you need to further de-serialize it into some kind of an object.
 ...
 * those objects asynchronously. Your higher-level library is built on top of
 * the original low-level library that does the RPC communication.
 * <p>
 * When the users of the higher-level library call {@code get}, you call
 * {@code get} on the lower-level library, which issues an RPC call and returns
 * a {@code Deferred} to the higher-level library. The higher-level library then
 * adds a first callback to further de-serialize the byte array into an object.
 * Then the user of the higher-level library adds their own callback that does
 * something with that object. So now we have something that looks like this:
 * 
 * <pre>
 *              1st callback                    2nd callback
 *
 ...
 * level library. All it does is, given an object that implements some interface
 * with a {@code get} method, it keeps a map of whatever arguments {@code get}
 * receives to an {@code Object} that was cached for this particular {@code get}
 * call. Thanks to the way the callback chain works, it's possible to use the
 * caching library together with the higher-level library transparently. Users
 * who want to use caching simply need to use the caching library together with
 * the higher level library. Now when they call {@code get} on the caching
 * library, and there's a cache miss, here's what happens, step by step:
 * <ol>
 * <li>The caching library calls {@code get} on the higher-level library.</li>
 * <li>The higher-level library calls {@code get} on the lower-level
 * library.</li>
 * <li>The lower-level library creates a {@code Deferred}, issues out the RPC
 * call and returns its {@code Deferred}.</li>
 * <li>The higher-level library adds its own object de-serialization callback to
 * the {@code Deferred} and returns it.</li>
 * <li>The caching library adds its own cache-updating callback to the
 * {@code Deferred} and returns it.</li>
 * <li>The user gets the {@code Deferred} and adds their own callback to do
 * something with the object retrieved from the data store.</li>
 * </ol>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Once the response comes back, the first callback is invoked, it de-serializes
 * the object, returns it. The <em>current result</em> of the {@code Deferred}
 * becomes the de-serialized object. The current state of the {@code Deferred}
 * is as follows:
 * 
 * <pre>
 ...
 *
 * <h2>Building dynamic processing pipelines with {@code Deferred}</h2> Let's
 * complicate the previous example a little bit more. Let's assume that the
 * remote storage service that serves those {@code get} calls is a distributed
 * service that runs on many machines. The data is partitioned over many nodes
 * and moves around as nodes come and go (due to machine failures and whatnot).
 * In order to execute a {@code get} call, the low-level client library first
 * needs to know which server is currently serving that piece of data. Let's
 * assume that there's another server, which is part of that distributed
 * service, that maintains an index and keeps track of where each piece of data
 * is. The low-level client library first needs to lookup the location of the
 * data using that first server (that's a first RPC), then retrieves it from the
 * storage node (that's another RPC). End users don't care that retrieving data
 * involves a 2-step process, they just want to call {@code get} and be called
 * back when the data (a byte array) is available.
 * <p>
 * This is where what's probably the most useful feature of {@code Deferred}
 * comes in. When the user calls {@code get}, the low-level library will issue a
 * first RPC to the index server to locate the piece of data requested by the
 * user. When issuing this {@code lookup} RPC, a {@code Deferred} gets created.
 * The low-level {@code get} code adds a first callback to process the
 * {@code lookup} response and then returns it to the user.
 * 
 * <pre>
 *              1st callback       2nd callback
 *
 ...
 * given the {@code lookup} response. So before triggering the first callback,
 * the {@code Deferred} will be in this state:
 * 
 * <pre>
 *              1st callback       2nd callback
 *
 ...
 * {@code (B)}, which is then returned by the {@code index lookup} callback. And
 * this is where the magic happens. Now we're in this state:
 * 
 * <pre>
 *   (A)        2nd callback    |   (B)
 *                              |
 ...
 * {@code Deferred}, they want it to receive a byte array. The current callback
 * gets <em>paused</em> and stops processing the callback chain. This callback
 * chain needs to be resumed whenever the {@code Deferred} of the {@code get}
 * call [{@code (B)}] completes. In order to achieve that, a callback is added
 * to that other {@code Deferred} that will <em>resume</em> the execution of the
 * callback chain.
 * 
 * <pre>
 ...
 * So the whole process of receiving the {@code lookup} response and sending out
 * the {@code get} RPC happened really quickly, without blocking anything.
 * <p>
 * Now when the {@code get} response comes back from the network, the RPC layer
 * de-serializes the byte array, as usual, and hands it to {@code (B)}:
 * 
 * <pre>
 *   (A)        2nd callback    |   (B)        1st callback
 *                              |
 ...
 * <p>
 * What we've done is essentially equivalent to dynamically building an implicit
 * finite state machine to handle the life cycle of the {@code get} request.
 * This simple API allows you to build arbitrarily complex processing pipelines
 * that make dynamic decisions at each stage of the pipeline as to what to do
 * next.
 *
 * <h2>Handling errors</h2> A {@code Deferred} has in fact not one but two
 * callback chains. The first chain is the "normal" processing chain, and the
 * second is the error handling chain. Twisted calls an error handling callback
 * an "errback", so we've kept that term here. When the asynchronous processing
 * completes with an error, the {@code Deferred} must be given the
 * {@link Exception} that was caught instead of giving it the result (or if no
 * {@link Exception} was caught, one must be created and handed to the
 * {@code Deferred}). When the current result of a {@code Deferred} is an
 * instance of {@link Exception}, the next errback is invoked. As for normal
 * callbacks, whatever the errback returns becomes the current result. If the
 * current result is still an instance of {@link Exception}, the next errback is
 * invoked. If the current result is no longer an {@link Exception}, the next
 * callback is invoked.
 * <p>
 ...
 * argument. Note that {@code Deferred} will only catch {@link Exception}s, not
 * any {@link Throwable} or {@link Error}.
 *
 * <a name="warranty"></a>
 ...
 * <a href="http://su.pr/2IB74L#64127">happens-before</a> the next callback is
 * invoked. In other words, if a callback chain manipulates a variable (and no
 * one else manipulates it), no synchronization is required.</li>
 * <li>The thread that executes the callback chain is the thread that hands the
 * initial result to the {@code Deferred}. This class does not create or manage
 * any thread or executor.</li>
 ...
 * {@code O(1)}.</li>
 * <li>A {@code Deferred} cannot receive itself as an initial or intermediate
 * result, as this would cause an infinite recursion.</li>
 * <li>You must not build a cycle of mutually dependant {@code Deferred}s, as
 * this would cause an infinite recursion (thankfully, it will quickly fail with
 * a {@link StackOverflowError}).</li>
 * <li>Callbacks and errbacks cannot receive a {@code Deferred} in argument.
 * This is because they always receive the result of a previous callback, and
 * when the result becomes a {@code Deferred}, we suspend the execution of the
 * callback chain until the result of that other {@code Deferred} is
 * available.</li>
 * <li>Callbacks cannot receive an {@link Exception} in argument. This because
 * they're always given to the errbacks.</li>
 * <li>Using the monitor of a {@code Deferred} can lead to a deadlock, so don't
 * use it. In other words, writing
 * 
 * <pre>
...
public final class Deferred<T> {
    ...
    /**
     * Maximum length of the callback chain we allow. Set this to some aggressive
     * limit to quickly detect problems in code that creates potentially infinite
     * chains. I can't imagine a practical case that would require a chain with more
     * callbacks than this.
     */
     ...
     * </pre>
     * 
     * A Deferred starts in the PENDING state, unless it's given a result when it's
     * instantiated in which case it starts in the DONE state. When
     * {@link #callback()} is invoked, the Deferred enters the RUNNING state and
     * starts to execute its callback chain. Once the callback chain is exhausted,
     * the Deferred enters the state DONE.
     *
     * While we're executing callbacks, if one of them returns another Deferred,
     * then we enter the state PAUSED, and we add a callback on the other Deferred
     * so we can resume execution (and return to the RUNNING state) once that other
     * Deferred is DONE.
     *
     * When we're DONE, if additional callbacks are added, we're going to execute
     * them immediately (since we already have the deferred result available), which
     * means we're going to return to the RUNNING state.
     */
    ...
    /**
     * The current state of this Deferred. All state transitions must be done
     * atomically. Since this attribute is volatile, a single assignment is atomic.
     * But if you need to do a state transition with a test first ("if state is A,
     * move to state B") then you must use {@link #casState} to do an atomic CAS
     * (Compare And Swap).
     */
     ...
     *
     * This reference doesn't need to be volatile because it's guaranteed that only
     * 1 thread at a time will access this reference, and all accesses happen in
     * between a volatile access on the {@link #state} variable.
     *
     * This reference isn't touched until the initial call to {@link #callback}
     * occurs. The thread that invokes {@link #callback} is going to run through the
     * callback chain in {@link #runCallbacks}, but it will first set the state to
     * RUNNING (volatile access). When the method returns, the state is changed
     * either to DONE or PAUSED (volatile access). When going out of PAUSED, the
     * state is set back to RUNNING before updating the result (volatile access).
     * Going from state DONE to RUNNING again also involves a volatile access.
     *
     * The new Java memory model thus guarantees that accesses to this reference
     * will always be consistent, since they always happen after a volatile access
     * to the state associated with this reference. See also the FAQ on JSR 133
     * (Java Memory Model) at http://su.pr/2WxCIb#volatile
     */
    private Object result;

    /**
     * The current callback chain (can be null). Invariants: - If callbacks is null,
     * errbacks is null too. - If state is State.PENDING, both lists may be null. -
     * If state is State.DONE, both lists must be null. - If callbacks is not null,
     * callbacks.size() == errbacks.size(). - null is not an allowed element in
     * either list. - All accesses to both lists must be done while synchronizing on
     * `this'.
     */
    ...
    private static final AtomicReferenceFieldUpdater<Deferred, State> stateUpdater = AtomicReferenceFieldUpdater
            .newUpdater(Deferred.class, State.class, "state");
     ...
     * @throws StackOverflowError if there are too many callbacks in this chain. The
     *                            maximum number of callbacks allowed in a chain is
     *                            set by the implementation. The limit is high
     *                            enough that you shouldn't have to worry about this
     *                            exception (which is why it's an {@link Error}
     *                            actually). If you hit it, you probably did
     *                            something wrong.
     */
    @SuppressWarnings("unchecked")
    public <R, R2, E> Deferred<R> addCallbacks(final Callback<R, T> cb, final Callback<R2, E> eb) {
        if (cb == null) {
        ...
        if (!casState(State.DONE, State.RUNNING)) {
            ...
            synchronized (this) {
                if (callbacks == null) {
                    ...
                    throw new StackOverflowError("Too many callbacks in " + this + " (size=" + callbacks.size()
                            + ") when attempting to add cb=" + cb + '@' + cb.hashCode() + ", eb=" + eb + '@'
                            + eb.hashCode());
                }
     ...
     * callback is executed immediately from this thread. If the deferred result is
     * already available and is an exception, the callback is discarded. If the
     * deferred result is not available, this callback is queued and will be invoked
     * from whichever thread gives this deferred its initial result by calling
     * {@link #callback}.
     * 
     * @param cb The callback to register.
     * @return {@code this} with an "updated" type.
     ...
     */
    @SuppressWarnings("unchecked")
    public <R, D extends Deferred<R>> Deferred<R> addCallbackDeferring(final Callback<D, T> cb) {
     ...
     * <p>
     * If the deferred result is already available and is an exception, the errback
     * is executed immediately from this thread. If the deferred result is already
     * available and isn't an exception, the errback is discarded. If the deferred
     * result is not available, this errback is queued and will be invoked from
     * whichever thread gives this deferred its initial result by calling
     * {@link #callback}.
     * 
     ...
     * immediately from this thread (regardless of whether or not the current result
     * is an exception). If the deferred result is not available, this callback is
     * queued and will be invoked from whichever thread gives this deferred its
     * initial result by calling {@link #callback}.
     * 
     * @param cb The callback to register.
     * @return {@code this} with an "updated" type.
     */
     ...
     */
    @SuppressWarnings("unchecked")
    public <R, D extends Deferred<R>> Deferred<R> addBothDeferring(final Callback<D, T> cb) {
        return addCallbacks((Callback<R, T>) ((Object) cb), (Callback<R, T>) ((Object) cb));
    }

    /**
     * Chains another {@code Deferred} to this one.
     * <p>
     * This method simply ensures that whenever the callback chain in {@code this}
     * is run, then the callback chain in {@code other} gets run too. The result
     * handed to {@code other} is whatever result {@code this} currently has.
     * <p>
     * One case where this is particularly useful is when you want to multiplex a
     * result to multiple {@code Deferred}s, that is if multiple "listeners" want to
     * have their callback chain triggered when a single event completes:
     * 
     * <pre>
     * {
     *     &#64;literal
     ...
     * <p>
     * Conceptually, this does the opposite of {@link #chain}, in the sense that it
     * demultiplexes multiple {@code Deferred}s into a single one, so that you can
     * easily take an action once all the {@code Deferred}s in the group have been
     * called back.
     * 
     * <pre>
     * {
     ...
     * </pre>
     * 
     * In the example above, any number of {@code Deferred} can be added to the
     * demultiplexer, and once {@code demultiplex()} is invoked, the
     ...
     *         {@code Deferred}s given in argument have been called back. Each
     *         element in the list will be either of type {@code <T>} or an
     *         {@link Exception}. If any of the elements in the list is an
     *         {@link Exception}, the errback of the {@code Deferred} returned will
     *         be invoked with a {@link DeferredGroupException} in argument.
     *         <p>
     *         There's no guarantee on the order of the results in the deferred list
     *         returned, it depends on the order in which the {@code Deferred}s in
     *         the group complete.
     */
    public static <T> Deferred<ArrayList<Object>> group(final Collection<Deferred<T>> deferreds) {
     ...
     * </pre>
     * 
     * except that it's type safe as it doesn't involve an unchecked generic array
     * creation of type {@code Deferred<T>} for the varargs parameter passed to
     * {@code asList}.
     * 
     ...
     */
    public static <T> Deferred<ArrayList<Object>> group(final Deferred<T> d1, final Deferred<T> d2) {
     ...
     * <p>
     * This posts the initial result that will be passed to the first callback in
     * the callback chain. If the argument is an {@link Exception} then the
     * "errback" chain will be triggered instead.
     * <p>
     * This method will not let any {@link Exception} thrown by a callback
     * propagate. You shouldn't try to catch any {@link RuntimeException} when you
     * call this method, as this is unnecessary.
     * 
    ...
    public void callback(final Object initresult) {
        if (!casState(State.PENDING, State.RUNNING)) {
            throw new AssertionError("This Deferred was already called!" + "  Current result=" + result
                    + ", new result given=" + initresult);
        }
        result = initresult;
     ...
     * <p>
     * This helps do synchronous operations using an asynchronous API. If this
     * Deferred already completed, this method returns (or throws) immediately.
     * Otherwise, the current thread will be <em>blocked</em> and will wait until
     ...
     * @throws Exception            if the deferred result is an exception, this
     ...
     * Deferred already completed, this method returns (or throws) immediately.
     * Otherwise, the current thread will be <em>blocked</em> and will wait until
     * the Deferred is called back. If the current thread gets interrupted while
     ...
     * 
     * @return The deferred result, at this point in the callback chain.
     * @throws Exception if the deferred result is an exception, this exception will
     *                   be thrown.
     */
     ...
     *                      interrupt us. If {@code false} then this method will
     *                      never throw {@link InterruptedException}, it will handle
     *                      it and keep waiting. In this case though, the
     *                      interrupted status on this thread will be set when this
     *                      method returns.
     * @return The deferred result, at this point in the callback chain.
     ...
     */
    @SuppressWarnings("unchecked")
    private T doJoin(final boolean interruptible) throws InterruptedException, Exception {
     ...
     *           don't know where this Deferred comes from (it was our initial
     *           result).
     * @throws AssertionError if {@code this == d} as this would cause an infinite
     *                        recursion.
     */
    @SuppressWarnings("unchecked")
     ...
     */
    private Callback<Object, Object> continueAfter(final Deferred d, final Callback cb) {
        return new Callback<Object, Object>() {
            public Object call(final Object arg) {
                if (arg instanceof Deferred) {
                    ...
                    final String cb2s = cb == null ? "null" : cb + "@" + cb.hashCode();
                    throw new AssertionError("Tried to resume the execution of " + Deferred.this
                            + ") although it's not in state=PAUSED." + "  This occurred after the completion of " + d
            ...
            public String toString() {
                return "(continuation of Deferred@" + Deferred.super.hashCode() + " after "
                        + (cb != null ? cb + "@" + cb.hashCode() : d) + ')';
            }
        };
    }
     ...
     * <p>
     * The string returned is built in {@code O(N)} where {@code N} is the number of
     * callbacks currently in the chain. The string isn't built entirely atomically,
     * so it can appear to show this {@code Deferred} in a slightly inconsistent
     * state.
     * <p>
    ...
    public String toString() {
        ...
        final StringBuilder buf = new StringBuilder((9 + 10 + 7 + 7 + result.length()) * 2);
        buf.append("Deferred@").append(super.hashCode()).append("(state=").append(state).append(", result=")
                .append(result).append(", callback=");
        synchronized (this) {
