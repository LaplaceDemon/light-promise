...
final class DeferredGroup<T> {
    ...
    /**
     * The Deferred we'll callback when all Deferreds in the group have been called
     * back.
     */
    private final Deferred<ArrayList<Object>> parent = new Deferred<ArrayList<Object>>();

    /** How many results do we expect? */
    private final short nresults;

    /**
     * All the results for each Deferred we're grouping. Need to acquires this'
    ...
    public String toString() {
        return "DeferredGroup" + "(parent=" + parent + ", # results=" + results.size() + " / " + nresults + ')';
    }
